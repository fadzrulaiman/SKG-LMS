        ...
        <div class="control-group">
            ...
            <div class="controls">
                <select name="role[]" multiple="multiple" size="3" required>
                    ...
                    <option value="<?php echo $roles_item['id'] ?>"
                        <?php if ($roles_item['id'] == 2) echo "selected"; ?>><?php echo $roles_item['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
        </div>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <select name="language">
                    ...
                    <option value="<?php echo $code; ?>" <?php if ($code == $default_lang) echo "selected"; ?>>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <select id="timezone" name="timezone" class="selectized input-xlarge">
                    ...
                    <option value="<?php echo $tz ?>" <?php if ($tz == $tzdef) echo "selected"; ?>><?php echo $tz; ?>
                    </option>
                    <?php
                        $index++;
                    } ?>
...
<script
    src="<?php echo base_url();?>assets/bootstrap-datepicker-1.8.0/locales/bootstrap-datepicker.<?php echo $language_code;?>.min.js">
</script>
<?php }?>
...
function select_manager() {
    ...
    if (employees.rows({
        ...
        var manager = employees.rows({
            selected: true
        }).data()[0][0];
        var text = employees.rows({
            selected: true
        }).data()[0][1] + ' ' + employees.rows({
            selected: true
        }).data()[0][2];
        $('#manager').val(manager);
        $('#txtManager').val(text);
...
function select_position() {
    ...
    if (positions.rows({
        ...
        var position = positions.rows({
            selected: true
        }).data()[0][0];
        var text = positions.rows({
            selected: true
        }).data()[0][1];
...
$(function() {
    ...
    $("#firstname").change(function() {
        var login = generateLogin($("#firstname").val(), $("#lastname").val(),
            '<?php echo $this->config->item('login_pattern')!==FALSE?$this->config->item('login_pattern'):'jdoe';?>',
            32);
        $("#login").val(login);
    });
            ...
            $.ajax({
                ...
                .done(function(msg) {
                    if (msg == "true") {
                        if ($('#contract').val() == "") {
                            bootbox.confirm(
                                "<?php echo lang('users_create_no_contract_confirm');?>",
                                function(result) {
                                    if (result == true) {
                                        submit_form();
                                    }
