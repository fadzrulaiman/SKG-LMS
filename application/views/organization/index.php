...
$(function() {
    ...
    $("#lnkDeleteEntity").click(function() {
        $.ajax({
                ...
                data: {
                    'entity': $('#frmConfirmDelete').data('id')
                }
            })
            .done(function(msg) {
    ...
    if (selectedData.length > 0) {
        var id = selectedData[0].id; // Access the 'id' property of the selected data
        console.log('Selected employee ID:', id); // Log the selected employee ID
        ...
        if (id != "") {
            if ($("#organization").jstree('get_selected').length == 1) {
                ...
                .done(function(msg) {
                    console.log('Server response:', msg); // Log the server response
                    if (msg.success) {
                        $('#frmModalAjaxWait').modal('show');
                        oTable.ajax.url("<?php echo base_url(); ?>organization/employees?id=" + entity)
                            .load(function() {
                        ...
                        alert('Failed to detach employee: ' + msg.error);
                    }
                })
    ...
    $('#organization').jstree({
        ...
        .on('create_node.jstree', function(e, data) {
            ...
            bootbox.prompt("<?php echo lang('organization_index_prompt_entity_name');?>",
                ...
                "<?php echo lang('organization_index_popup_node_button_ok');?>",
                function(result) {
